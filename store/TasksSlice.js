...
const TasksSlice = createSlice({
  ...
  reducers: {
    ...
    changeStatus(state, action) {
      const [currentTask] = state.tasks.filter((task) => task.id === action.payload);
...
export const selectAlltasks = (state) => state.tasksInfo.tasks.filter((task) => !task.isCompleted);
export const selectCompletedTasks = (state) => state.tasksInfo.tasks
  .filter((task) => task.isCompleted);
export const selectCategorys = (state) => state.tasksInfo.category;
export const selectFilter = (state) => state.tasksInfo.filter;

export const {
  initial, add, remove, setFilter, changeStatus, addCategory, removeCategory,
} = TasksSlice.actions;
